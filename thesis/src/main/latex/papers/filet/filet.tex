...
\documentclass[9pt,journal,compsoc,twocolumn,twoside]{IEEEtran}
%\documentclass[10pt,journal,compsoc,draftcls,onecolumn,peerreview]{IEEEtran}
%
...
{(Blinded)%\MakeLowercase{\textit{et al.}
...
% page as needed:
\ifCLASSOPTIONpeerreview
  \begin{center} \bfseries EDICS Category: ADP-PPRO \end{center}
\fi
%
...
The NSA collected network data with a program called XKeyscore. The documents leaked by Snowden (e.g., \cite{XKeyscore}) suggest details of this application. According to these papers, XKeyscore spanned (in 2008) $\approx150$ sites with $700$ Severs collecting emails, web traffic, and chat messages.
...
As a next step, the sender assembles a VortexMessage. The VortexMessage contains the full message, parts of the message, and possibly decoy payload. Furthermore, it contains the previously built routing block and some additional information in a header required for the protocol (such as the workspace). 

The Vortex message is then passed from VortexNode to VortexNode. Each VortexNode executes the set of instructions in the allocated workspace with the received message parts. 
...
So, our protocol allows the passing of message fragments through a mesh of VortexNodes. Each node is aware of the sender of a message and the receiver of the processed result. However, none of the nodes is aware where the message is destined to, or if the passed message contains payload or just decoy traffic. The load put on the network may increase between the node randomly and unrelated to any payload content. Furthermore, by routing back parts of the traffic to the sending node, we may monitor the progress of message delivery or even identify misbehaving VortexNodes. Due to the nature of the Reed-Solomon function, we are even able to build redundancy in the message path or guarantee that regardless of the sanity of some of the involved VortexNodes, the message may be rebuilt at the receiving node.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}[ht]
	\centering\includegraphics[width=\textwidth]{roughProtocolDesign}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}[ht]
	\includegraphics[width=\textwidth]{blockLayoutSimplified}
...
The second key is the peer key $K_{peer_o}$ preceding the encrypted $\mathbf{HEADER}$ block. The RBB chooses the key. This key protects the inner structure of the message. It makes it impossible for any node except the sending or the receiving peer node to detect the inner structure of the message. Without this key, any independent observer with knowledge about the blending capabilities of a receiving node may:
\begin{itemize}
...
\begin{figure}[ht]
	\centering\includegraphics[width=\columnwidth]{roughProtocolDesign_workspace}
...
\begin{figure}[ht]\centering
	\includegraphics[width=0.8\columnwidth]{addRedundancyOp}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[ht]
	\includegraphics[width=\columnwidth]{messagePaths}
...
This paper is a very rough overview of the MessageVortex protocol. For those interested in the technical implementation details the current version of the RFC\ifCLASSOPTIONpeerreview\cite{Blinded}\else{}\cite{MessageVortexRFC}\fi. For a more elaborated analysis covering additional topics such as the blending types, additional literature research, or arguments for a decision, we recommend reading the thesis paper \ifCLASSOPTIONpeerreview\cite{Blinded}\else{}\cite{messageVortex}\fi. In this document we cover additional details such as elaborated analysis on the protocol, the implications of the connection between transport endpoints and VortexNodes, or an analysis of the plain embedding technique.
\ifCLASSOPTIONpeerreview
\hfill (Blinded)
\else
\hfill gwm
