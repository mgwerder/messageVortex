...
A VortexMessage is passed from one router to another. This Vortexmessage is embedded as binary data in the transfer protocol. Every router may decide for himself on the support of algorithms and embedding mechanisms.
...
Things do however drastically change if taking a human censor into account. \fxwarning{Add more text here}
...
A VortexMessageis not identifiable as the message is structureless on the outside. This means that the message itself follows immediately the encrypted key without any structure. Therefore, we require the hosts private key to tell whether there is VortexMessage within a transport message.

The padding block $PAD$ makes sure that even if a routing block is reused, the messagge structure is not the same. however, the preceeding block with the key remains the same

An active adversary may not create own routing blocks or header blocks and inject them due to the forward secret. He may however replace the peer key of a message. As this key is anyway known to him no additional knowledge is gained there. Replacing the sender key block will break the message. Replacing the header or routing block of the message with another header or routing block from the same ephemeral identity will break the message unless the sender key and the forward secret is reused. Finally, exchanging, omitting, or adding payload blocks will render the message inoperable but not generate additional knowledge. Replying the same or a modified block will not generate any pattern on the network as the replay protection will stop propagating messages at the next node.
...
Furthermore some issues have been identified relating to updating nodes. A node should be able to request the software over VortexMessages as official sources for updates may be blocked. The presence of such a routing node should be protected with a small platform featuring deniable encryption and anti forensic measures. We are currently investigating the possibility of creating such a cheap platform based on an RaspberryPi Zero.
