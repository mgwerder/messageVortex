...
of the University of Applied Sciences Northwestern Switzerland.\protect\\
...
Whistleblower Edward Snowden leaked a vast amount of documents. These documents suggested that such attacks on privacy are commonly made on a global scale. The documents leaked in 2009 and a significant number of journalists from multiple countries screened them (e.g., \cite{NCR2013}, \cite{XKeyscore}, \cite{Ball2013} or \cite{Greenberg2013}). According to these documents, Natinal Security Agency (NSA) infiltrated more than 50k computer networks with malware to collect classified or personal information. They furthermore infiltrated Telecom-Operators (according to the same documents mainly executed by British GCHQ) such as Belgacom to collect data and targeted high member of governments even in associated states (such as the mobile phone number of Germany's president and high ranking members of economy and finance).

The NSA collected network data with a program called XKeyscore (e.g., \cite{XKeyscore}). According to the leaked papers, XKeyscore spanned (in 2008) $\approx150$ sites with $700$ Severs collecting emails, web traffic, and chat messages.
...
People must be able to control their data footprint. Not providing thhis control does effectively allow any country or a bigger player to ban and control any number of persons within or outside the Internet.  

\section{Formalization}
In this work, a new protocol is designed to allow message transfer through existing communication channels. These messages are next to unobservable for any third party. This unobservability does not only cover the message itself but all metadata and flows associated with it. We called this protocol ``MessageVortex'' or in short just ``Vortex''. The protocol is designed in such a way so that it is capable of using a wide variety of transport protocols. It is even possible to switch transfer protocols for the same message in transfer. This behavior allows media breaches (at least on a protocol level) and makes analysis of the protocol by an adversary even harder.

\subsection{What We Worked On}
The new protocol allows secure communication without the need for trusting the underlying transport media. Furthermore, the usage of the protocol itself is possible without altering the immediate behavior of the transport layer. That way, it is possible to use the transport layers regular traffic to increase the noise in which potential information has to be searched by an adversary. 

We send messages between nodes by hiding a message in a common transport protocol in the internet (e.g., SMTP) by so called routing nodes. The routing nodes take instructions on how to process the message from the message itself and send the processed information including an encrypted set of processing instructions to the next nodes. The instructions available are encryption/decryption, splitting and merging, and add/remove redundancy information by applying a Reed-Solomon operation. To avoid collisions when sending messages a sender first allocates a workspace on each node used to route the message. Each workspace holds message parts and instructions on how to process them for a very limited amount of time. The message parts are represented in the workspace as IDs whereas the first IDs have special meanings (e.g., a block unfolding to ID 0 is destined for the current node). 

Either instructions provided in the message itself or received and triggered afterwards by the same routing block builder may be used to assemble messages for the current node or neighboring routing nodes.

A message typically includes from the very start all information to process the message to the end. Every routing node is just processing ``its part'' without any knowledge of what it is processing. A routing node is unaware if generated additional message content by Reed-Solomon operations is used for redundancy or decoy traffic generation.

Diagnosis or even reputation may be built by routing intermediate results of the message transfer back to the sender. If message delivery fails, a sender may create additional messages picking up such intermediate results for diagnosis.

We did not only implement the instruction part of the protocol, but a holistic protocol approach trying to mitigate misuse of the system by design. While it is not yet production grade it features all parts of the protocol.
...
We refer to jurisdiction as a geographical area where a set of legal rules created by a single actor or a group of actors apply and this geographical area contains, furthermore, the executive means to enforce this set of legal rules.
...
As notation for asymmetric encryption we use $E^{K^{1}_a}(\mathbf{M})$ where as $K^{-1}_a$ is the private key and $K^{1}_a$ is the public key of a key pair $K^{p_a}=\langle K^1_a, K^{-1}_a\rangle$. The asymmetric decryption is noted as $D^{K^{-1}_a}(\mathbf{M})$.
...
In general, subscripts denote selectors to differentiate values of the same type or specifications, and superscript denotes relevant parameters to operations expressed. The subscripted and superscripted information may be omitted where not needed.
...
While this representation is perfectly true from the mathematical point of view, we used for the implementation a symmetrical key $K_b$ to encrypt the blocks which was protected by an asymmetric key pair, which then results in $\mathbf{M}=D^{D^{K^{-1}_a}(K_b)}\left(\mathbf{MESSAGE}^{K_b}\right)$. For a more precise description of the message see \ref{sec:messageOutline}.
...
As a definition for censorship, we take a definition which is attributed to Chuck Stone Professor at the School of Journalism and Mass Communication, University of North Carolina. 
\begin{quote}
	Censorship: The cyclical suppression, banning, expurgation, or editing by an individual, institution, group or government that enforce or influence its decision against members of the public -- of any written or pictorial materials which that individual, institution, group or government deems obscene and ``utterly without redeeming social value,'' as determined by ``contemporary community standards.''
\end{quote}

Please note that ``Self Censorship'' (not expressing something in fear of consequences)  is a form of censorship according to this definition too.
...
\DeclareFixedFootnote{\omitted}{footnotes of quotes are ommited in citations for clarity}
...
We were unable to identify a protocol withstanding the definition of our strong adversary. We have, however, found many protocols dealing with anonymity and very few dealing with censorship circumvention. We learned from these approaches and integrated useful approaches wherever we were able to. It has to be said, however, that very little of the mentioned protocols here had ever experienced broad adoption. Even more, most of them were never implemented or challenged. 
...
\begin{itemize}
	\item \emph{Hide data}\\
	The most common approaches we have found were either mimicking protocols (as in \cite{mohajeri2013skypemorph}), use protocols as payload transports (as in \cite{AthanRAM07}), employ steganography (as in \cite{f5}) or comparable technologies as side channels.
	...
	Censorship measurements, especially regarding the Internet censorship of China, have been analyzed in depth under technological, sociological and economical aspects (see\cite{Ensafi_2015}, \cite{Clayton_2006}, or \cite{lowe2007great} to name a few)
\end{itemize}

For Anonymization the ideas seem mainly to concentrate around onionizing (ToR\cite{tor-spec}, SOR\cite{Egners_2012}, DUO-Onions and Hydra-Onions\cite{iwanik2005duo}), Dining Cryptographer networks (DC-Nets; e.g., \cite{chaum-dc}, Tarzan\cite{tarzan:ccs02}, GAS\cite{AthanRAM07}), mixing (Babel\cite{babel}, MorphMix\cite{morphmix:wpes2002}, Mixminion\cite{minion-design}, Salsa\cite{Salsa}) and distributed hash tables (DHT; e.g., Bifrost\cite{Kondo2009} or BitBlender\cite{Bauer_2008}).
...
In this section, we introduce a new consistent, transport independent model for representing the different protocols used by MessageVortex. The focus of the description lies in academic concepts. For more technical information specifying the protocol as implemented, we refer to \cite{MessageVortexRFC}. 
...
The transport layer is a common message passing protocol on the Internet. This infrastructure lies within the public part of the Internet but is not modified for serve this protocol. It offers a store-and-forward infrastructure for the other protocol layers. Although we used SMTP for our experiments, it is not limited to this protocol. The RFC draft document also specifies XMPP. We refer to the upper part (accounting, routing, and blending layer) of each node as VortexNode. These nodes may be any device with a permanent connection to the Internet (e.g., a RaspberryPi computer or a mobile phone). The message paths shown in the figure are not relevant. Any path layout such as cyclical or tree-like may be possible.
...
A VortexMessage is passed from one router to another and is embedded as binary data in the transfer protocol. Every Vortex node may decide for itself on the support of algorithms and embedding mechanisms.

A VortexMessage contains an encrypted symmetric key $E^{K_{host_n}}\left(K_{peer_n}\right)$ immediately followed by an inner message $E^{K_{peer_n}}$ which is encrypted with this key. The inner message contains a series of blocks encoded in ASN.1. In Figure~\ref{fig:messageOutline} we show a simplified view on a VortexMessage. 

The block structure of a Vortex message is as follows:
\begin{itemize}
	\item \emph{Encrypted peer key $K_{peer_n}$}\\
	...
	\item \emph{Header block encrypted with $K_{sender_n}$}\\
	This block contains all information about the sender and is signed with the identities private key (detached signature)
	\begin{itemize}
		\item \emph{Identity}\\
		This information contains the public key of the identity. It serves as an identifier for the workspace and is authenticated by the signature.
	...
	This block contains all the information required for processing the payload blocks and reassemble new messages.
	\begin{itemize}
		...
		These routing blocks are placed into the VortexMessage created according to the build instructions and are already encrypted with $k_{sender_{a+1}}$ and thus not readable to the current node.
		\item Next hop header (encrypted with $k_{sender_{a+1}}$)
		These header blocks are placed into the VortexMessage created according to the build instructions and are already encrypted with $k_{sender_{a+1}}$ and thus not readable to the current node.
		...
		\item \emph{Next hop peer key $k_{peer_{a+1}}$}\\
...
The second key is the peer key $k_{peer_n}$ located in the encrypted $headerBlock$. The RBB chooses the key. This key protects the inner structure of the message. It makes it impossible for any node except the sending or the receiving peer node to detect the inner structure of the message. Without this key, any independent observer with knowledge about the blending capabilities of a receiving node may:
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[ht]
	\includegraphics[width=\columnwidth]{../../inc/messagePaths}
