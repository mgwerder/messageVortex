...
All encryption steps do leak some properties. They may leak the algorithm due to the block size. The chosen parameter may be unique to the RBB. If randomly chosen, this is no longer the case. If chosen by an implementation-specific pattern, the pattern may leak the implementation over time. As the analysis must be done over a short period (the lifetime of an \defref{eID}), it is up to an RBB to leak as little information as possible. We do regard the cryptographically secured content as secure. 
...
A sender may have some knowledge about the Routing block size and, therefore, guess the routing path's complexity. He is, however, unable to gain any additional information such as time of travel or number of hops until the target.
...
%FIXME: is that statement really true or just delaying?
...
\chapter[Degree of Anonymization in Comparison]{Analysis of Degree of Anonymization in Comparison to other Systems}
