...
This dialing technology enabled automatic messaging for voice and text messages (e.g. telex) up until today and is the foundation for today's routed networks. 
...
Furthermore -- Any central infrastructure is bound to be of special interest to anyone gathering data concerning the using entities of such a protocol. It furthermore may be manipulated in order to attack the messages or their flow. So central infrastructure have to be avoided.

\marginpar{What is the problem?}Leaving no information trail when sending information from one person to another is hard to achieve. Most messaging systems disclose at least the peer partners when sending messages. Metadata such as starting and endpoints, frequency, or message size are leaked in all common systems even when encrypting messages.

\marginpar{Who is involved?} This affects senders and recipients of information in the internet and allows a potent adversary capable of collecting a vast amount of data to build a throughout profile of a person. Unlike in the past the availability of this kind of information has been risen to a never known extend with the internet.

\marginpar{Where does it occur?} These profiles are used in many places. These include service adoption, directed advertising or classification of citizens. The examples given above show that effects of this data is not limited to the internet but reaches us effectively in the real world.

\marginpar{When does it occur?} The main problem of this data is that it may be collected over a huge amount of time and evaluated at any time. It even happens that common practices of a time are suddenly disregarded and persons judged retrospectively as seen in the tax avoidance discussion. 

\marginpar{Why is it important?} People must be able to control their data footprint. Not providing these means does effectively allow any country or bigger player to ban and control any number of persons within or outside the internet. 

In this work a new protocol is designed to allow message transfer through existing communication channels. These messages are next to unobservable to any third party. This unobservability does not only cover the message itself but all metadata and flows associated with it. We called this protocol ``MessageVortex'' or in short just ``Vortex''. The protocol is designed in such a way so that it is capable to use a wide variety of transport protocols. It is even possible to switch protocols while the messages are in transfer. This allows media breaches (at least on a protocol level) and makes analysis even harder.
