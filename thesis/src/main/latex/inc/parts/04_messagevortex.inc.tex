...
As a result of the architecture of today's common networks, the anonymization of a sender or a receiver is not simple. A relay may allow at least the anonymization of the original sender given trust into such infrastructure. %FIXME2 Reworded
...
In a generic sense, a node may also replay a message to highlight a messages property (e.g., the path or a size), which may lead to the discovery of such meta information. 

%FIXME2 reworded section
...
These operations build the core of the routing capabilities of a node. The operation allows an \defref{RBB} to add redundancy to a message or parts of it (payload chunk) information to a message or to rebuild a block from a chosen set of information.  %FIXME2 reworded paragraph

The operation itself is shown in \cref{fig:addRedundancyOperation}. 

\begin{figure}[ht]\centering
	\resizebox{0.51\textwidth}{!}{\input{inc/addRedundancyOp.inc.tex}}
