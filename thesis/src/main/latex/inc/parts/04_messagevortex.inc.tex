...
Accessing cryptographically weakly protected data is possible. However, accessing or modifying such data typically requires a high amount of calculation resources. Such resources may be available for a single case, but they typically do not scale if we assume high protocol usage,
...
%FIXME this makes no sense, are you suspicious of the infrastructire? Please revise
...
As a result of the architecture of today's common networks, the anonymization of a sender or a receiver is not simple. A relay may allow at least the anonymization of the original sender given trust into the proxy. %FIXME Do you mean given that there is reust in the proxy?
...
In a generic sense, a node may also replay a message to highlight a messages property (e.g., the path or a size), which leads us to the following requirement:%FIXME see section right below
...
A node always consists of three layers and one or more transports connected to it. The understanding of their inner workings is essential to the understanding of the project as a whole. %FIXME Repetitive see above - key components
...
This concept has certain downsides related to the expiration of \defref{eID}s. We will address them in \cref{sec:newEID} and \ref{sec:replaceMURB}.
...
The accounting layer keeps tracks of all information required and assigned to ephemeral identities (\defref{eID}). It is queried by the blending and the routing layer for the authorization of the operations. The accounting layer manages the following tuples of information:
...
These operations build the core of the routing capabilities of a node. The operation allows an \defref{RBB} to add redundancy %FIXME do you mean adding redundant information or information about redundancy?
...
The routing itself is done in a temporarily allocated storage called ``\defref{workspace}'', which is tied to an ephemeral identity (\defref{eID}) represented by an asymmetric key pair. To obtain an \defref{eID}, a sender typically solves a crypto puzzle. 
