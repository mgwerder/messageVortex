...
In this part, we describe the core parts of the \MessageVortex{} protocol. Unlike most other academic attempts, we do this on the base of an adversary, which is capable of banning our technology. We, therefore, are not able to focus solely on the anonymity property. Instead, we first collect requirements for such a system in \cref{sec:genRequirements}. Based on these requirements, we explain our architectural concepts and decisions in \cref{sec:rationale}. We then build an outline of our protocol focusing on the protocol's main properties, without going too much into implementation details. In \cref{sec:protocol}, we describe the protocol and its key concepts in depth. We explain all aspects relevant to the academic solution without going into implementation details. The implementation details are described in the RFC draft document in \cref{app:rfcMessageVortexMain}. Additionally, we describe academically relevant details of the implementation, as well as their realization in infrastructure, in \cref{sec:implementation}. For operational concerns such as route building strategies, refer to \cref{sec:operation}.

\chapter{Requirements for an Anonymizing Protocol}\label{sec:genRequirements}
In the following sections, we first define a threat model. We then elaborate on the main characteristics of the anonymizing protocol based on the threat model. This allows us to build a coherent model for our target protocol.
...
\begin{table*}[ht]\centering\tiny
	...
	\begin{tabular}{|l|l|l|p{9.8cm}|}\hline
...
The adversaries defined above are not realistic bu in a way ``worst case assumptions''. It is possible that an adversary monitors certain spots within a network. Such spots are typically either jurisdictional borders or neuralgic points within a jurisdiction such as the central router of an Internet sevice provider (ISP). It is, however, not realistic, that an adversary is able to tap any point of a network in the scale of a jurisdiction. Such taping would require almost infinite bandwidth and unlimited access. 

Accessing cryptographically weakly protected data is possible. However, accessing or difying such data requires typically a high amount of calculation resources. Such resources may be available for a single case but they typically do not scale if we assume high protocol usage,

Modifying network traffic would require even higher evolved capabilities as such modification requires not only tapping of a network but the capability of actively modifying the network traffic. Such modification is in practice almost always limited to a broadcast domain. This means that all devices within a broadcast domain receive always the same messages. In our model we actually state that the trffic may  be modified at any point within the jurisdiction freely. This is not a realistic assumption underlying todays common network technologies.

It is furthermore not realistic that a state-sponsored-actor is carrying out a DDoS attack against an entity within a jurisdiction as simply blocing traffic would be far more effective and less ressource binding. DDoS attack may however be a good solution when aiming to  disrupt services within a jurisdiction not cooperating with the adversaries goals.

An adversary may, however have a limited number of accesses to the network with exactly these capabilities. As we are unable to define or limit a number of access points, our defined adversaries reflect a worst case assumption which may not be surpassed. Therefore, our adverasaries are, while not realistic, reflect a state where, if our protocol withstands such adversaries, may be considered as safe.

\section{Required Properties for our Unobservable Protocol}\label{sec:properties}
In this section, we collect required properties for our system. We first list a property and then explain why it is essential.
...
Users are unable to limit the route of network packets through named jurisdictions. Therefore, we must protect users of \MessageVortex{} (users) from being subject to legal prosecution in any jurisdiction. All these users need to be anonymous when sending or receiving messages. This applies not only to their communication but as well to the usage of an anonymizing technology. Unfortunately, most transport protocols (in fact, all of the common ones such as \defref{SMTP}, SMS, \defref{XMPP}, or IP) use a globally unique identifier for senders and recipients, which are readable by any party which is capable of reading the packets (mainly the routing nodes).  
...
To withstand an adversary outlined in \cref{sec:adversary}, the messages sent even within the system requires to be unidentifiable by meta information or content. ``Meta information'' may include any information including, but not limited to, frequency, timing, message size, sender, protocol, ports, or recipient. If we want to guarantee that a node is not identifiable as an endpoint of a message, all involved nodes must carry out equivalent \defref{operation}s. As soon as we have differences between routing nodes and endpoints, we can identify participating persons at entry or exit nodes.

If we want users traffic to remain undistinguishable from traffic generated from routing nodes, all traffic muist have the same properties. This applies not only after ``entering the system'' but any time. As a result only an infrastructure-less aproach may be used as a consequence. A hybrid or server based approach requires infrastructure to be placed within the Internet. Jurisdictions with a censoring adversary may place focus on such systems and identify and prosecute their owners. 

Furthermore, it must be impossible from an observing adversary to identify message endpoints. All nodes must look equal from the outside in terms of traffic, as well as in terms of their offered functions and behavior. The term ``Equal nodes'' does not necessarily mean that nodes must be indistinguishable. It merely means that given the functions, capabilities, and behavior of a node, no further information can be deduced and no differentiation in function may be done.
...
Unobservability requires, according to \cite{anonTerminology}, that an item of interest (\defref{IoI}) is undetectable from an uninvolved entity and anonymous for the involved entities. We, therefore, require anonymization as a property.
...
\ref{req:zeroTrust} implies that a node may try to introduce such features into the message. As we cannot keep a node from doing so, we can define that such tags must be removed by the next node. This may only be done if any node apart from the sender and recipient node have no access to the message being transported or the message is protected from modification.
...
This bootstrapping is needed for users to extend their network at first to a reasonable anonymity set towards an adversary inside the system. At the same time, the bootstrapping  mechanism is a great danger as it allows an adversary to harvest nodes. As a result, each node must be able to control by whom a node is discoverable.
...
\begin{enumerate}
	...
	\item the infrastructure provides the service, as opposed to: ``is running in a degraded or disfunctional state and, therefore, possibly unable to provide the service.''
\end{enumerate}
...
We did not consider efficiency as a requirement. Our goal is to achieve anonymity under harsh conditions such as . 
...
Important properties of such piggybacking are mainly:
\begin{itemize}
...
\begin{itemize}
	...
	\item Run offers routing \defref{operation}s instead of traditional mixing and recombination methods.
...
\begin{figure}[ht]
	\resizebox{0.95\textwidth}{!}{\input{inc/layerDesign.inc.tex}}
...
\begin{itemize}
	\item \textbf{\defref{sender}:} The user or process originally composing the message. in contrast to the \defref{sender}, the \defref{immediate sender} is the node that was sending the message to the current node. It may or may not be identical to the \defref{sender}.
	...
	\item \textbf{\defref{eID}:} An identity created on any node with a limited lifetime anyone possessing the private key (proven by encrypting with it) is accepted as representative of that identity. An \defref{eID} has a \defref{workspace} associated to it. Please note that an \defref{eID}{} is not identical to an \defref{ID}{} which is a numerical identifier for a payload block storage location within a workspace.
...
% FIXME continue reading from here
...
We first describe a single node and its identity. This node is always equivalent to a potential sender, recipient or a . 

We then introduce the concept of \defref{workspace}s and ephemeral identities (\defref{eID}s). These concepts are essential for the routing and accounting layers. They dictate memory and storage requirements, and lay a foundation for the routing layer.
...
The protocol itself has the possibility to answer clear-text requests. So-called ``public nodes'' (see \cref{sec:vortexNodeTypes}) make use of such messages. They are, however, an exception. In general, all \VortexMessages{} are encrypted. 
...
A routing layer needs to receive all payload and routing blocks, and process them (For an exact outline of the routing block, see \cref{sec:vortexMessage}). These blocks are stored in a suitable list within the \defref{workspace} of the \defref{eID} identified by the header block.
...
\begin{figure}[ht]
	\centering\resizebox{.95\linewidth}{!}{
		\input{inc/vortexMessage.inc.tex}
	}
...
After authorization by the accounting layer, the header block is processed as outlined in \cref{sec:processingIncommingMessages}. Basically, we just add the routing blocks and payload to the respective \defref{workspace} and wait for the routing layer to process the information.
...
\subsubsection{\texorpdfstring{$addRedundancy$}{addRedundancy} and \texorpdfstring{$removeRedundancy$}{removeRedundancy} Operations}\label{sec:redundancyOperation}
...
The Lagrange functions have been proposed in \cite{shamir1979share} and further more generalized in \cite{mceliece1981sharing} for sharing secrets. The general idea about all proposed schemes is to distribute informations and restrict access to it so that only if a specified number of shares are captured a secret may be rebuilt. Unlike proposed in these papers we do not apply privacy to our protocol by sharing the data among many points. Instead we use Lagrange functions to create decoy traffic. By doing so even a creator of traffic is unable to tell message traffic from decoy traffic apart. 
...
\begin{figure}[ht]\centering
	\resizebox{0.48\textwidth}{!}{\input{inc/addRedundancyOp.inc.tex}}
	\resizebox{0.45\textwidth}{!}{\input{inc/removeRedundancyOp.inc.tex}}
...
\begin{figure}[ht]\centering
	\resizebox{0.35\textwidth}{!}{\input{inc/encrypt.inc.tex}}
	\resizebox{0.35\textwidth}{!}{\input{inc/decrypt.inc.tex}}
...
\begin{figure}[ht]\centering
	\resizebox{0.43\textwidth}{!}{\input{inc/split.inc.tex}}
	\resizebox{0.35\textwidth}{!}{\input{inc/merge.inc.tex}}
