...
\begin{eqnarray}
i          & = & len(\mathbf{M})\\
e          & = & blocksize\left(E^{K}\right) \cdot n\\
...
\mathbf{X} & = & \langle p,\mathbf{M},R_{t}\left(s,l-i\right)\rangle\\\nonumber
           & = & \langle p,\mathbf{M},R_{t}\left(s,p \pmod{len\left(\mathbf{X}\right)-len(p)}\right)\rangle
\end{eqnarray}    
...
To reverse a successful message recovery information of a padded block $\mathbf{X}$, we calculate the original message size by extracting $p$ and doing $i=len(\mathbf{M})=p \pmod{ len(\mathbf{X}-len(p))}$.
