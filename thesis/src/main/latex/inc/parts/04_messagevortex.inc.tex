...
After authorization by the accounting layer, the header block is processed as outlined in \cref{sec:processingIncommingMessages}. Basically, we just added the routing blocks and payload to the respective \defref{workspace} and waited for the routing layer to process the information.
...
\subsubsection{The \texorpdfstring{$addRedundancy$}{addRedundancy} and \texorpdfstring{$removeRedundancy$}{removeRedundancy} Operations}\label{sec:redundancyOperation}
