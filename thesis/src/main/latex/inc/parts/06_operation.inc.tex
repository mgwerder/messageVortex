...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Involved keys or parameters.
	\end{itemize}
...
Every forseeable pattern may be used to identify \VortexMessages{} or in transport involved nodes. This is why we cannot use a fixed pattern in routing. Instead we use randomized routing paterns. 
...
In the following section we show a simple algorithm for creating a routing graph in a non-censored environment or in an isolated node set in a censored environment. While the algoprithm is complete we had to short it for \fxwarning{incomplete text}
