...
\begin{itemize}
	\item Assuming an adversary has partial or full insight into a \defref{routing graph} (except for the sender and the final recipient), all operations must be valid. This means that no operation may be applied and an inverse operation with different parameters  (i.e., $D^{K_a}\left(E^{K_a}\left(\textbf{X}\right)\right)$).
...
\begin{figure*}[ht]\centering
  \begin{tikzpicture}[node distance=0.5cm,auto,>=stealth]
	\begin{scope}[baseline=(current bounding box.center)]
		...
		\draw[->] (r0)--(r3);
		\draw[->] (r1)--(r2);
		\draw[->,bend right,looseness=0.4] (r0) edge (r1);
		\draw[->] (r3)--(r4);
		\draw[->,bend right,looseness=0.4,in=190] (r0) edge (r3);
		\draw[->] (r3)--(r5);
		\draw[->] (r0)--(r6);
		\draw[->] (r3)--(r1);
		\draw[->] (r5)--(r1);
		\draw[->,bend right,looseness=0.4] (r3)edge(r0);
		\draw[->] (r5)--(r0);
		\draw[->] (r6)--(r4);
		\draw[->,bend right,looseness=0.4,out=-15,in=190] (r5) edge (r0);
		\draw[->] (r2)--(r6);
		\draw[->] (r3)--(r2);
		\draw[->] (r2)--(r5);
	\end{scope}
	\begin{scope}[baseline=(current bounding box.center),xshift=5cm,yshift=2cm]
		...
		% Legend
		\draw[->,shorten <=25pt,shorten >=20pt] (nm0) -- (nm0Ground)  node[midway,sloped,left,rotate=90] {$\text{time}$};
		
	\end{scope}
	\draw[->,shorten <=25pt,shorten >=20pt,draw=red] ($(r0)!0.0!(r0)$)--($(n0)!0.4!(n0Ground)$);
\end{tikzpicture}
...
\begin{breakablealgorithm}
	...
	\begin{algorithmic}[1]
		...
		\Object{Universe}
			...
			\LineComment{Returns a random node of knownNodes}
...
\fxwarning{rewrite algorithm for oo type of algorithm}
...
\begin{breakablealgorithm}
	...
	\begin{algorithmic}[1]
		...
		\EndFunction
		\item[]		
		\Function{assignRoute}{route, payloadIds, lastNode, targetIds}\funclabel{alg:assignRoute}\label{alg:startAssignRoute-line}
