...
\partepigraph{Where does a snake's tail start?}{My Son}
...
\begin{itemize}
	\item Hide data by\ldots
	\begin{itemize}
		\item obfuscation or encryption
		\item applying steganographic methodes
	\end{itemize}
...
The following sections list two PRNG specifications to follow the recommendations of \cite{rfc1750}. These PRNGs are used to complete the padding in some of the later specified operations.
...
\fxwarning{Extend PRNG section. Where is the rest?} 
...
Zero trust is not an acdemically defined concept. In fact it is widely misused by many marketing departments of well known devices and applications related to security.

We however define zero trust as follows:
\begin{quote}
	We assume that any participant in a network not explicitely trusted may:
	\begin{itemize}
		\item Make some or all information available to him available to others.
		\item Analyze all information within his reach.
		\item Willingly break protocol rules in order to gain information or other advantages.
	\end{itemize}
\end{quote}

As design principle this means that information is kept hidden as much as possible within the system. 

\fxwarning{incomplete section}
