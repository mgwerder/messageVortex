...
% allow breaking within math when using comma (breaks cv)
%\makeatletter
%\def\old@comma{,}
%\catcode`\,=13
%\def,{%
...
\begin{abstract}
In this paper we intoduce the MessageVortex protocol. An unobservable message annonymisation protocol. It is based on the zero trust principle and a distributed peer-to-peer (P2P) architecture and avoids  central aspects such as fixed infrastructures wuithin a global network. It scores over existing work by blending its traffic into suitable existing transport protocols thus making it next to impossible to block it without affecting significantly regular users of the transport medium. It furthermore requires no protocol specific infrastructure in public networks and allows a sender to control all aspects of a message such as degree of anonymity, timing, and redundancy of the message transport without disclosing any of these details to the routing or transporting nodes.
\end{abstract}
