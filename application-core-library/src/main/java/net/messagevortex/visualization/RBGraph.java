...
import net.messagevortex.generator.AnonymitySet;
import net.messagevortex.generator.Parameter;
...
public class RBGraph extends ZoomPanel {
    ...
    private final TreeMap<Integer, Timeslot> timeslots;
    ...
    private void getNodes() {
        ...
        AnonymitySet.getNodes().forEach((nKey, nValue) -> {
            Node node = new Node(nValue.getID(), nodeId.get());
        ...
        routingblocks.forEach((rbKey, rbValue) -> nodes.forEach((nodeKey, nodeValue) -> {
    ...
    private TreeMap<Integer, Timeslot> getTimeslots() {
        TreeMap<Integer, Timeslot> slots = new TreeMap<>();
        ...
        routingblocks.forEach((key, value) -> {
            ...
            slots.put(startTime, new Timeslot(startTime, endTime, value));
        });
    ...
    private void drawRhombuses(TreeMap<Integer, Timeslot> timeslots, Graphics2D g2d) {
        for(Integer key : timeslots.keySet()) {
            g2d.setColor(new Color(69, 93, 133));
            Polygon rhombus = drawRhombus(timeslots.get(key).getRb().getSenderId(), timeslots.get(key));
            graphedObjects.put(graphedObjectsIterator, new GraphObject(rhombus.xpoints, rhombus.ypoints, timeslots.get(key).getRb().getRoutingCombo().dumpValueNotation("", DumpType.INTERNAL), GraphObjectType.RHOMBUS));
    ...
    private void drawRoutes(TreeMap<Integer, Timeslot> timeslots, Polygon rhombuses, Graphics2D g2d) {
        for(Integer key : timeslots.keySet()) {
            ...
            if(h > 50) {
                ...
                for(int j = (h / 50); j > 0; j--) {
                    int sender = timeslots.get(key).getRb().getSenderId();
                    int recipient = timeslots.get(key).getRb().getRecipientId();
                    int height = 250 + (spacingTimeSlots * timeslots.get(key).getSlot()) - yPoints[0];
                    ...
                    p1 = new Point(startPointGraph, 250 + (spacingTimeSlots * timeslots.get(key).getSlot()));
                    p2 = new Point(endpointGraph, 250 + (spacingTimeSlots * timeslots.get(key).getSlot()));
                }
            } else {
                int sender = timeslots.get(key).getRb().getSenderId();
                int recipient = timeslots.get(key).getRb().getRecipientId();
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(int i = 0; i < nodes.size(); i++) {
            ...
            graphedObjects.put(graphedObjectsIterator, new GraphObject(xPos, 100, 100, 100, "Information", GraphObjectType.RECTANGLE));
            graphedObjectsIterator++;
        ...
        timeslots.forEach((key, value) -> g2d.drawString(value.getTimeString(), 80, 250 + spacingTimeSlots * value.getSlot()));
    }

    @Override
    public Dimension getPreferredSize() {
        double width = (300 + (spacingNodes * nodes.size())) * zoomFactor;
        double height = (500 + (spacingTimeSlots * timeslots.size())) * zoomFactor;
