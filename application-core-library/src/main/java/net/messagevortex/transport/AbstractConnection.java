...
public abstract class AbstractConnection {
  ...
  private int readRawSocket(long timeout) throws IOException {
    ...
    try {
      ...
      LOGGER.log(Level.INFO, "done reading raw socket (took "
    ...
    inboundEncryptedData.flip();
    LOGGER.log(Level.INFO, "reverting inbound buffer (new size is "
  ...
  private int writeRawSocket(long timeout) throws IOException {
    ...
    try {
      ...
      LOGGER.log(Level.INFO, "sending outboundEncryptedData Buffer done ("
              + outboundEncryptedData.remaining() + ")");
  ...
  private void processEngineRequirements(long timeout) throws IOException {
    ...
    LOGGER.log(Level.INFO, "status (1) outboundEncryptedData Buffer "
    ...
    while (!shutdownAbstractConnection && handshakeStatus != HandshakeStatus.FINISHED
      ...
      switch (handshakeStatus) {
        case NEED_UNWRAP:
          ...
          switch (res.getStatus()) {
            case OK:
              LOGGER.log(Level.INFO, "unwrap done (OK;inboundAppData="
            ...
            case BUFFER_UNDERFLOW:
              ...
              handshakeStatus = getEngine().getHandshakeStatus();
              LOGGER.log(Level.INFO, "unwrap done (UNDERFLOW; inboundEncryptedData Buffer "
            ...
            case BUFFER_OVERFLOW:
              ...
              handshakeStatus = getEngine().getHandshakeStatus();
              LOGGER.log(Level.INFO, "unwrap done (OVERFLOW; inboundAppData Buffer "
        ...
        case NEED_WRAP:
          ...
          outboundEncryptedData.flip();
          LOGGER.log(Level.INFO, "doing wrap (outboundEncryptedData is "
                  + outboundEncryptedData.remaining() + "/"
          ...
          switch (result.getStatus()) {
            ...
            case BUFFER_OVERFLOW:
              ...
              LOGGER.log(Level.INFO, "wrap done (OVERFLOW; outboundEncryptedData Buffer "
                      + outboundEncryptedData.remaining() + "/"
                      + outboundEncryptedData.capacity() + ")");
              outboundEncryptedData.compact();
              handshakeStatus = result.getHandshakeStatus();
      ...
      outboundEncryptedData.flip();
      LOGGER.log(Level.INFO, "status outboundEncryptedData Buffer "
              + outboundEncryptedData.remaining() + "/" + outboundEncryptedData.capacity() + "");
      outboundEncryptedData.compact();
      outboundAppData.flip();
      LOGGER.log(Level.INFO, "status outboundAppData Buffer " + outboundAppData.remaining()
              + "/" + outboundAppData.capacity() + "");
      outboundAppData.compact();
      LOGGER.log(Level.INFO, "status inboundEncryptedData Buffer "
              + inboundEncryptedData.remaining() + "/" + inboundEncryptedData.capacity() + "");
      LOGGER.log(Level.INFO, "status inboundAppData Buffer " + inboundAppData.remaining()
  ...
  private int readSocket(long timeout) throws IOException, TimeoutException {
    ...
    do {
      ...
      if (inboundEncryptedData.remaining() > 0) {
        if (isTls()) {
          ...
          inboundAppData.flip();
          bytesRead += inboundAppData.remaining();
          LOGGER.log(Level.INFO, "decryption done (occupied buffer space: "
        ...
        if (bytesRead > 0) {
          ...
          LOGGER.log(Level.INFO, "got bytes (" + bytesRead + " bytes; occupied buffer space: "
                  + inboundEncryptedData.remaining() + ")");
        }
    ...
    if (bytesRead < 0) {
      ...
      LOGGER.log(Level.INFO, "Loop aborted due to data (" + totBytesRead
