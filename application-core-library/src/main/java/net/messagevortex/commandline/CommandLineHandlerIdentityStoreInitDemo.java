...
public class CommandLineHandlerIdentityStoreInitDemo implements Callable<Integer> {
  ...
  public Integer call() throws Exception {
    ...
    if (new File(CommandLineHandlerIdentityStore.DEFAULT_FILENAME).exists()) {
      LOGGER.log(Level.SEVERE, "File \"" + CommandLineHandlerIdentityStore.DEFAULT_FILENAME + "\" exists already");
      return MessageVortex.ARGUMENT_FAIL;
    }

    // create store
    LOGGER.log(Level.INFO, "creating identity store \"" + CommandLineHandlerIdentityStore.DEFAULT_FILENAME + "\"");
    int ret = MessageVortex.mainReturn(new String[] {"store", "create"});
    if (ret != 0) throw new IOException("Error while creating a new store");
    ...
    for (int i=1; i<=9; i++) {
      ...
      if (ret != 0) throw new IOException("Error while creating identity store");
    ...
    IdentityStore is = new IdentityStore(new File(CommandLineHandlerIdentityStore.DEFAULT_FILENAME));
